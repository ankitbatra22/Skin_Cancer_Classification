...
class HAM10000Dataset(Dataset):
    ...
    def _oversample(self, oversample_rates):
        """Oversample or undersample classes based on the given rates."""
        new_dfs = []
        for i, rate in enumerate(oversample_rates):
            ...
            if rate > 1:  # Oversampling
                ...
                new_dfs.append(class_df)  # Keep the original class data
                new_dfs.append(oversampled_class)  # Add oversampled data
            else:  # Undersampling (reduce to 20% of original size)
                undersampled_class = class_df.sample(
                    n=int(len(class_df) * rate),
                    random_state=42
                )
                new_dfs.append(undersampled_class)
        
        # Combine all class-specific DataFrames
